...
public class NoticeService {
    ...
    public NoticeDto[] selectNotices(NoticeDto noticeDto) {
        NoticeEntity noticeEntity = NoticeEntity.noticeDtoToNoticeEntity(noticeDto);
        String orderBy = noticeDto.getOrderBy();
        int limit = noticeDto.getLimit();
        int page = noticeDto.getPage();

        // orderBy, limit, page 값이 예상된 범위를 벗어나지 않는지 검증한다.
        // 만약 벗어난다면 서버에 대한 공격일 가능성을 있으므로 간단한 로그를 남긴다.
        if (!orderBy.equals("user_id") ||
        ...
        int count = noticeMapper.findNoticesByNoticeWithoutData(noticeEntity);
        ...
        NoticeEntity[] noticeEntities = noticeMapper.findNoticesByNotice(noticeEntity,
                orderBy,
                limit,
                offset);
    ...
    public int selectNoticesByNoticeWithoutData(NoticeDto noticeDto) {
        NoticeEntity noticeEntity = NoticeEntity.noticeDtoToNoticeEntity(noticeDto);
        int count = noticeMapper.findNoticesByNoticeWithoutData(noticeEntity);
        return count;
    }
