    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        switch (provider) {
            case "kakao":
                ...
                log.info("OAuth2UserService loadUser() attributes :\n"
                        + attributes);
        ...
        if (dbUserEntity == null) {
            ...
            log.info("OAuth2UserService loadUser() 신규 회원 : " + providerId);
        } else {
            // DB에 있을 시, 기존 User 정보 할당
            userDto = UserDto.userEntityToUserDto(dbUserEntity);
            log.info("OAuth2UserService loadUser() 기존 회원 : " + providerId);
        }
