...
mainForm['infoEmailSend'].onclick = function () {
    ...
    if (!new RegExp(mainForm['infoEmail'].dataset.regex).test(mainForm['infoEmail'].value)) {
        dialog.show({
            ...
            buttons: [
                dialog.createButton('확인', function () {
                    mainForm['infoEmail'].focus();  // 커서가 해당 위치로 가게 함.
                    mainForm['infoEmail'].select(); // 안에 글자가 있으면 선택한 상태로 만든다.
                    dialog.hide();
                })
            ]
        });
    ...
    const xhr = new XMLHttpRequest();  //이메일 인증번호 전송을 요청하기 위한 XHR 객체
    const formData = new FormData(); // 요청을 보낼 때 이메일을 같이 보내기 위한 FormData 객체
    ...
    xhr.open('POST', './registerEmail'); //get 방식으로 보낼 때는 뒤에 ?
    xhr.send(formData);
    loading.show();
}
...
mainForm['infoEmailVerify'].onclick = function () {
    ...
    };// get은 정보를 받을 때, post는 정보를 가져올 때, patch는 정보를 수정할 때 주로 사용한다.
    xhr.open('PATCH', './registerEmail'); // xhr.open에서 PATCH와 ./registerEmail를 통해 무슨 값을 받아올지 판단한다.
    xhr.send(formData); // 서버로 formData 에 있는 정보를 내보내겠다.
}

mainForm['infoAddressFind'].onclick = function () {
    new daum.Postcode({ // 다음주소 api를 통해서 사용가능하다.
        width: '100%',
        height: '100%',
...
mainForm.onsubmit = function (e) {
    ...
    switch (parseInt(mainForm.dataset.step)) {
        ...
        case 2:
            ...
            xhr.onreadystatechange = function () {
                ...
                switch (responseObject['result']) {
                    ...
                    case 'failure_duplicate_nickname':
                        dialog.show({
                            title: '경고',
                            content: '해당 닉네임은 이미 사용중입니다.<br><br>다른 닉네임을 입력해 주세요.',
                            buttons: [dialog.createButton('확인', function () {
                                dialog.hide();
                                mainForm['infoNickname'].focus();
                                mainForm['infoNickname'].select();
                    ...
                    case 'failure_duplicate_contact':
                        dialog.show({
                            title: '경고',
                            content: '해당 연락처는 이미 사용중입니다.<br><br>다른 연락처를 입력해 주세요.',
                            buttons: [dialog.createButton('확인', function () {
                                dialog.hide();
                                mainForm['infoContactFirst'].focus();
                                mainForm['infoContactFirst'].select();
            ...
            xhr.open('POST', './register'); // 회원가입 할 때 insert가 생기므로 POST로 사용
            xhr.send(formData);
            loading.show();
            break;
        case 3:
